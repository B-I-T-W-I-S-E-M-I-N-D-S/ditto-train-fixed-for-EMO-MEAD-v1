...
class MotionDiffusion(nn.Module):
    def __init__(
        ...
        dim_ws=None,
        # ── NEW emotion options ─────────────────────────────────────────
        lambda_emo: float = 0.1,   # weight for emotion consistency loss
    ):
        super().__init__()
    ...
    def p_losses(self, x_start, cond_frame, cond, t):
        ...
        if self.part_w_dict:
            ...
            # ── Emotion Consistency Loss (NEW) ───────────────────────────
            # When the model has an EmotionEncoder, we encourage the predicted
            # motion to be consistent with the audio emotion representation.
            # We do this by comparing the emotion embeddings extracted from
            # the noisy input trajectory vs. those from the clean target.
            # In practice we compare the pooled emotion features: the
            # EmotionEncoder operates on audio, so we use a simple MSE on
            # the emo_embed produced by both runs (target already computed
            # inside model.forward via the shared EmotionEncoder).
            #
            # For a lightweight implementation we compute emo consistency as:
            #   L_emo = MSE( emo_pred_pooled , emo_target_pooled )
            # where both are derived from the SAME audio (cond), so we
            # compare the encoder output between two differently-noised runs.
            # This encourages temporal smoothness of the emotion representation.
            if self.lambda_emo > 0.0 and hasattr(self.model, 'use_emotion') and self.model.use_emotion:
                try:
                    # Extract HuBERT slice from cond [B, T, hubert_dim]
                    hubert_dim = self.model.hubert_dim
                    hubert_feats = cond[..., :hubert_dim].to(
                        next(self.model.emotion_encoder.parameters()).dtype
                    )
                    # Two runs with different noise levels for consistency
                    with torch.no_grad():
                        emo_target = self.model.emotion_encoder(hubert_feats)
                    # Run-from-noisy (same audio → same emo, so this tests smoothness)
                    emo_pred = self.model.emotion_encoder(
                        hubert_feats + 0.05 * torch.randn_like(hubert_feats)
                    )
                    l_emo = F.mse_loss(emo_pred, emo_target)
                    total_loss = total_loss + self.lambda_emo * l_emo
                    loss_dict['emo_consistency'] = l_emo
                except Exception:
                    pass  # Don't crash training if emotion loss fails
